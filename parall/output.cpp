...
static const char* run_mode_name(int run_mode) {
  return (run_mode == RUN_MODE_VALIDATION) ? "validation" : "perf";
}

static int parse_run_mode(const char* value) {
  if (!strcmp(value, "0") || !strcmp(value, "perf")) return RUN_MODE_PERF;
  if (!strcmp(value, "1") || !strcmp(value, "validation")) return RUN_MODE_VALIDATION;
  return -1;
}
...
void write_params_to_log(const char* filename) {
  ...
  fprintf(f, "run_mode=%s\n", run_mode_name(param.run_mode));
  fprintf(f, "kmc_time_rule=batch_avg_dt\n");
  fprintf(f, "conflict_policy=defer_to_next_step\n");
  fprintf(f, "load: %s\n", param.load);
...
void write_to_log(const char* filename) {
  ...
  double bad_pct = 0.0;
  double back_pct = 0.0;
  if (current.n_jumps > 0) {
    bad_pct = (current.n_bad_jumps * 100.0) / current.n_jumps;
    back_pct = (current.n_jumps_back * 100.0) / current.n_jumps;
  }
  f=fopen(filename,"at");
  if (f==NULL) {fprintf(stderr,"!!! cannot open log file '%s'\n",filename); return;}
  fprintf(f, "%.2f ML	t: %.2f s	rate: %.3f ML/s (type %d, T %.0f)	deposited: %6d	jumps: %10d	(bad %.0f%%, back %.0f%%)	moves: %12.0f\n",
    ...
    current.n_deposited, current.n_jumps, bad_pct, back_pct, current.n_moves);
  fclose(f);
}
...
void read_parameters(const char* filename) {
  ...
  while (1) {
    ...
    if (!strcmp(buf,"run_mode")) {
      int mode = parse_run_mode(buf2);
      if (mode < 0) {
        fprintf(stderr,"!!! unknown run_mode value '%s' (use perf/validation or 0/1)\n", buf2);
        exit(0);
      }
      param.run_mode = mode;
      continue;
